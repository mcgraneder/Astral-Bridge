...
const BalanceDisplayInner = ({
    ...
    return (
        <div
            className={`my-5 flex flex-col items-center rounded-lg border border-tertiary p-2 text-center ${
                assetPrice ? 'min-h-[110px]' : 'min-h-[96px]'
            }`}
        >
            {!isNative ? (
            ...
            {isNative ? (
                <GlowingText loading={fetchingBalances}>
                    ...
                    {balance ? (
                        <span>{` ${CHAINS[chainId!]?.symbol}`}</span>
                    ) : null}
                </GlowingText>
            ) : (
                <GlowingText loading={fetchingBalances}>
